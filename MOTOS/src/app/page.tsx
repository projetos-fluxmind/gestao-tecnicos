...
export default async function Dashboard() {
  const [techs, motos, oilChanges] = await Promise.all([
    getTechnicians(),
    getMotorcycles(),
    getOilChanges()
  ]);

  const activeTechs = techs.filter((t: any) => t.status === 'ativo').length;
  const totalMotos = motos.length;
  const availableMotos = motos.filter((m: any) => m.status === 'ativa').length;
  const maintMotos = motos.filter((m: any) => m.status === 'em_manutencao').length;

  // Calculate critical oil alerts
  const oilAlerts = motos.filter((m: any) => {
  ...
  const stats = [
    ...
    { label: 'Disponibilidade', value: `${availableMotos}/${totalMotos}`, icon: Bike, trend: 'Frota Ativa', color: 'brand-emerald' },
    { label: 'Em Manutenção', value: maintMotos.toString(), icon: Wrench, trend: 'Oficina', color: 'brand-orange' },
    { label: 'Total Ativos', value: totalMotos.toString(), icon: TrendingUp, trend: 'Frota Total', color: 'brand-cyan' },
  ];
        ...
        <div className="lg:col-span-2 space-y-8">
          <div className="glass rounded-3xl overflow-hidden shadow-2xl">
            ...
            <div className="divide-y divide-white/5">
              {motos.slice(0, 5).map((moto: any) => {
                const currentAssignment = techs.find((t: any) => t.assignments.some((a: any) => a.motoId === moto.id));
                return (
                  <div key={moto.id} className="p-6 flex items-center justify-between hover:bg-white/2 transition-all group">
                    <div className="flex items-center gap-5">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-foreground/30">{moto.modelo} • <span className="text-brand-cyan/60 font-medium">{currentAssignment?.nome || 'Disponível'}</span></p>
                      </div>
                    </div>
