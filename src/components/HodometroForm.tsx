...
export function HodometroForm({ motos, techs }: { motos: any[], techs: any[] }) {
    ...
    async function handleSubmit() {
        ...
        formData.append("km", cleanKm.toString());
        formData.append("tipo", "outro"); // padrão por enquanto

        const res = await createOdometerReading(formData);

        if (res?.success) {
            setMsg({ type: 'success', text: 'KM registrado com sucesso!' });
            setMotoId("");
            setKm("");
        } else {
            setMsg({ type: 'error', text: res?.error || 'Erro ao salvar. Verifique se o valor é válido.' });
        }
        setLoading(false);
    }
